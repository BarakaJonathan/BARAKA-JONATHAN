      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center gap-12">
        <div className="flex-1 text-center md:text-left space-y-6">
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${isSpace ? 'bg-blue-900/30 border-cyan-500/50 text-cyan-300' : 'bg-slate-800 border-slate-700 text-emerald-400'}`}>
            <Terminal size={14} />
            <span className="text-sm font-medium tracking-wide">Professional Software Development</span>
          </div>
        ...
        <div className="flex-1 relative flex justify-center">
            <div className={`relative w-72 h-72 md:w-96 md:h-96 rounded-full flex items-center justify-center ${isSpace ? 'animate-float' : ''}`}>
               {/* Decorative Circles */}
